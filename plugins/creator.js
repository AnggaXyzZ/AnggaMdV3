...
let name = await conn.getName(who)
 ...
 const ini = await conn.sendContactArray(m.chat, [
    [`${nomorown}`, `${await conn.getName(nomorown+'@s.whatsapp.net')}`, `👑 Developer Bot `, `🚫 Don't call me 🥺`, `⁨angga007ps3@gmail.com`, `🇮🇩 Indonesia`, `🚀 https://youtube.com/channel/UCY89Fb9pCVUeMqEGikvOhNQ`, `nothing`],
    [`${nomorown2}`, `${await conn.getName(nomorown2+'@s.whatsapp.net')}`, `👑 Developer Bot `, `🚫 Don't call me 🥺`, `⁨-`, `🇮🇩 Indonesia`, `-`, `nothing`],
    [`${conn.user.jid.split('@')[0]}`, `${await conn.getName(conn.user.jid)}`, `🔥 Bot WhatsApp 🐣`, `📵 Don't spam/call me 😢`, `Nothing`, `🇮🇩 Indonesia`, `🚀 https://youtube.com/channel/UCY89Fb9pCVUeMqEGikvOhNQ`, `🤖 Dont Forget To Donate!`]
  ], fkontak)
